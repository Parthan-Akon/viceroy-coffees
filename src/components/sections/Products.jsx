...
function RatioBar({ ratio, featured }) {
  return (
    <div className="mb-6">
      <div className="flex h-2 w-full overflow-hidden rounded-full bg-primary/10">
        <div className="h-full bg-primary" style={{ width: `${ratio.arabica}%` }} />
        <div className="h-full bg-accent" style={{ width: `${ratio.robusta}%` }} />
      </div>
      <div className="mt-2 flex justify-between text-[11px] uppercase tracking-wide text-primary/50">
      ...
      <div className="grid grid-cols-1 gap-6 sm:grid-cols-2 lg:grid-cols-4">
        {products.map((p, i) => (
          <RevealOnScroll key={p.id} delay={i * 0.08} className={p.featured ? "lg:-mt-4" : ""}>
            <div
              className={`flex h-full flex-col rounded-md p-8 transition-shadow duration-300 ${
                p.featured
            ...
            >
              ...
              <h3 className="mb-4 font-display text-2xl font-medium">{p.name}</h3>

              <div className={p.featured ? "[&_.text-primary\\/50]:text-cream/60" : ""}>
                <RatioBar ratio={p.ratio} featured={p.featured} />
              </div>
